...
async fn list_notes(
    ...
    if filters.count_only.unwrap_or(false) {
        ...
        let counts = NoteCount { total, starred, deleted };
    ...
    Ok(ApiResponse::list(serde_json::to_value(notes).unwrap(), total, page, per_page))
}
...
async fn create_note(
    ...
    if let Some(ref tag_names) = body.tag_names {
        for name in tag_names {
            ...
            sqlx::query("INSERT INTO note_tags (note_id, tag_id) VALUES ($1, $2) ON CONFLICT DO NOTHING")
                .bind(note.id)
                .bind(tag_id)
                .execute(&mut *tx)
                .await?;
...
async fn toggle_star(
    ...
    Ok(ApiResponse::ok(serde_json::json!({ "is_starred": is_starred })))
}
